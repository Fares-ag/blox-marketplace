...
import { buildIjarahAgreementPdf } from '../src/applications/documents/agreement-pdf';
...
async function main() {
  const t0 = Date.now();
  const pdf = await buildIjarahAgreementPdf(ctx);
  fs.writeFileSync('test-ijarah.pdf', pdf);
