...
exports.buildKycVerificationSummary = buildKycVerificationSummary;
function latestResult(results, kind) {
    let found;
    for (const row of results) {
        if (row.kind !== kind)
            continue;
        if (!found || new Date(row.created_at) > new Date(found.created_at))
            found = row;
    }
    return found;
}
...
function humanizeReason(code) {
    if (code.startsWith('DIDIT_SESSION:'))
        return `Session ${code.slice('DIDIT_SESSION:'.length)}`;
    if (code.startsWith('DIDIT_'))
        return code.slice('DIDIT_'.length).replace(/_/g, ' ').toLowerCase();
    return code.replace(/_/g, ' ').toLowerCase();
}
function isDiditProvider(results) {
    return results.some((r) => r.model_version?.startsWith('didit/'));
}
const ID_DOC_TYPES = ['qid_front', 'qid_back', 'passport'];
function idDocumentCheck(kase) {
    const docs = kase.documents.filter((d) => ID_DOC_TYPES.includes(d.type));
    if (docs.length === 0) {
...
function authenticityCheck(kase) {
    const auth = latestResult(kase.verification_results ?? [], 'authenticity');
    if (!auth) {
        return { score: null, passed: null, status: 'not_submitted', reasons: [] };
    }
...
function collectWarnings(results, didit) {
    const warnings = new Set();
    for (const row of results) {
        for (const reason of parseReasons(row.reasons)) {
            if (reason.startsWith('DIDIT_SESSION:') || reason.startsWith('DIDIT_'))
                continue;
...
function parseDiditSession(results) {
    for (const row of results) {
        for (const reason of parseReasons(row.reasons)) {
...
function overallStatus(kase, checks) {
    ...
    if (allChecks.every((c) => c.status === 'not_submitted'))
        return 'not_started';
    if (allChecks.some((c) => c.status === 'failed'))
        return 'declined';
...
function mergeDiditVendorStatus(check, step) {
    if (!step?.status)
        return check;
...
function buildKycVerificationSummary(kase, kycStatus) {
...
//# sourceMappingURL=kyc-verification-summary.js.map