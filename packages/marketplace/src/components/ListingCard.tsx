...
import { MoneyText, formatQar, getAppLocale, formatCardFacets, hasWarranty, labelCondition } from '@drivemarket/shared';
          ...
          {product.company_code ? (
            <span
              ...
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                window.location.href = `/dealers/${product.company_code}`;
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
